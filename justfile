...
blog: && (_rename_md_mdx "blog")
    rm -rf blog/*
    npx @sillsdev/docu-notion@0.13.4 --log-level debug -n {{NOTION_TOKEN}} -r {{NOTION_BLOG_ROOT}} --status-tag '*' --markdown-output-path $(pwd)/blog

# Generate docs from notion https://github.com/sillsdev/docu-notion
docs: && (_rename_md_mdx "docs")
    rm -rf docs/*
    npx @sillsdev/docu-notion@0.13.4 --log-level debug -n {{NOTION_TOKEN}} -r {{NOTION_DOCUMENT_ROOT}} --status-tag '*' --markdown-output-path $(pwd)/docs
